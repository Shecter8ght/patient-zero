...
func capture() -> void:
	...
	# Кластер смертельных попаданий + пара укусов вокруг точки.
	for _k in 6:
		var off := Vector2(randf_range(-1.6, 1.6), randf_range(-1.6, 1.6))
		var dir := Vector2(randf_range(-1.0, 1.0), randf_range(-1.0, 1.0)).normalized()
		game.blood_hit.emit(at + off, fl, true, dir)
	for _k in 4:
		var off := Vector2(randf_range(-2.4, 2.4), randf_range(-2.4, 2.4))
