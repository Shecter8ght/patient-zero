...
func _ready() -> void:
	...
	var sim_node := get_node_or_null("/root/Main")
	if sim_node and sim_node.has_signal("bark_event"):
		sim_node.bark_event.connect(_on_bark_event)


func _on_bark_event(agent: int, cat: String, pos2: Vector2, fl: int) -> void:
	var y := MapGen.floor_y3d(fl)
	var world_pos := Vector3(pos2.x, y + 1.2, pos2.y)
	say(agent, cat, world_pos)
